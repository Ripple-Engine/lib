...
public:
    ...
    std::optional<T> pop_and_get_front_if_possible() {
        std::unique_lock<std::shared_mutex> g0(mut_head);
        std::unique_lock<std::shared_mutex> g1(mut_tail);
    ...
    bool empty() const {
        std::shared_lock<std::shared_mutex> g0(mut_head);
        return empty_nonlocking();
    }

    bool full() const {
        std::shared_lock<std::shared_mutex> g0(mut_tail);
